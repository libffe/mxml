        ...
        mxml_save_cb_t cb) /* I - Whitespace callback or MXML_NO_CALLBACK */
{
  ...
  if ((s = malloc(bytes + 1)) == NULL) return (NULL);
        ...
        void *sax_data)       /* I - SAX user data */
{
  ...
  return (
      mxml_load_data(top, (void *)&s, cb, mxml_string_getc, sax_cb, sax_data));
}
...
void mxmlSetWrapMargin(
    int column) /* I - Column for wrapping, 0 to disable wrapping */
{
  _mxml_global_t *global = _mxml_global();
  /* Global data */
                 ...
                 int *encoding) /* IO - Encoding */
{
  ...
  if (buf->current >= buf->end)
    if (mxml_fd_read(buf) < 0) return (EOF);
  ...
  switch (*encoding) {
    case ENCODE_UTF8:
      ...
      if (!(ch & 0x80)) {
        ...
        if (mxml_bad_char(ch)) {
          mxml_error(
              "Bad control character 0x%02x not allowed by XML standard!", ch);
          return (EOF);
        }
        ...
        *encoding = ENCODE_UTF16LE;
        ...
        if (ch == 0xfeff) return (mxml_fd_getc(p, encoding));
    ...
    case ENCODE_UTF16BE:
      ...
      if (mxml_bad_char(ch)) {
        ...
        if (lch < 0xdc00 || lch >= 0xdfff) return (EOF);
                 ...
                 void *p) /* I - File descriptor buffer */
{
  ...
  if (buf->current >= buf->end)
    if (mxml_fd_write(buf) < 0) return (-1);
    ...
    mxml_fd_write(_mxml_fdbuf_t *buf) /* I - File descriptor buffer */
{
  ...
  if (buf->current == buf->buffer) return (0);
  ...
  for (ptr = buf->buffer; ptr < buf->current; ptr += bytes)
    if ((bytes = write(buf->fd, ptr, buf->current - ptr)) < 0) return (-1);
    ...
    case ENCODE_UTF8:
      ...
      if (!(ch & 0x80)) {
        ...
        *encoding = ENCODE_UTF16LE;
        ...
        if ((temp = getc(fp)) == EOF || (temp & 0xc0) != 0x80) return (EOF);
        ...
        if (ch == 0xfeff) return (mxml_file_getc(p, encoding));
      } else if ((ch & 0xf8) == 0xf0) {
        /*
         * Four-byte value...
         */
...
/* I  - Get character function */
{
  ...
  *entptr = '\0';
  ...
  if (mxml_bad_char(ch)) {
    mxml_error(
        "Bad control character 0x%02x under parent <%s> not allowed by XML "
        "standard!",
        ...
        case MXML_CUSTOM:
          if (global->custom_load_cb) {
            ...
            if ((*global->custom_load_cb)(node, buffer)) {
              mxml_error("Bad custom value '%s' in parent <%s>!", buffer,
                         parent ? parent->value.element.name : "null");
              mxmlDelete(node);
              node = NULL;
            }
    ...
    if (ch == '<') {
      ...
      while ((ch = (*getc_cb)(p, &encoding)) != EOF)
        if (mxml_isspace(ch) || ch == '>' || (ch == '/' && bufptr > buffer))
          ...
          if (mxml_add_char(ch, &bufptr, &buffer, &bufsize)) goto error;
      ...
      if (!strcmp(buffer, "!--")) {
        ...
        *bufptr = '\0';
        ...
        if (!parent && first) {
          ...
          mxml_error("<%s> cannot be a second root node after <%s>", buffer,
        ...
        *bufptr = '\0';
        ...
        if (mxml_isspace(ch)) {
          ...
          if ((ch = (*getc_cb)(p, &encoding)) != '>') {
            mxml_error("Expected > but got '%c' instead for element <%s/>!", ch,
                       buffer);
            mxmlDelete(node);
            goto error;
          }
                       ...
                       _mxml_getc_cb_t getc_cb) /* I  - Data callback */
{
  ...
  while ((ch = (*getc_cb)(p, encoding)) != EOF) {
    ...
    if (ch == '/' || ch == '?') {
      ...
      if (quote != '>') {
        mxml_error("Expected '>' after '%c' for element %s, but got '%c'!", ch,
                   node->value.element.name, quote);
        goto error;
      }
    ...
    if (ch == '\"' || ch == '\'') {
      ...
      while ((ch = (*getc_cb)(p, encoding)) != EOF) {
        ...
        if (mxml_add_char(ch, &ptr, &name, &namesize)) goto error;
    ...
    *ptr = '\0';
    ...
    if (mxmlElementGetAttr(node, name)) goto error;

    while (ch != EOF && mxml_isspace(ch)) ch = (*getc_cb)(p, encoding);
    ...
    if (ch == '=') {
      ...
      if (ch == EOF) {
        mxml_error("Missing value for attribute '%s' in element %s!", name,
                   node->value.element.name);
        goto error;
      }
      ...
      if (ch == '\'' || ch == '\"') {
        ...
        while ((ch = (*getc_cb)(p, encoding)) != EOF)
          if (ch == quote)
            ...
            if (mxml_add_char(ch, &ptr, &value, &valsize)) goto error;
          }
      ...
      case ENCODE_UTF8:
        if (!(ch & 0x80)) {
          ...
          *encoding = ENCODE_UTF16LE;
          ...
          if (ch == 0xfeff) return (mxml_string_getc(p, encoding));
...
/* I - Write callback */
{
  ...
  if (*s == '\"' || *s == '\'') {
    ...
    if ((*putc_cb)(*s, p) < 0) return (-1);

    quote = *s++;

    while (*s && *s != quote) {
      if ((name = mxmlEntityGetName(*s)) != NULL) {
        if ((*putc_cb)('&', p) < 0) return (-1);

        while (*name) {
          if ((*putc_cb)(*name, p) < 0) return (-1);
  ...
  switch (node->type) {
    case MXML_ELEMENT:
      ...
      for (i = node->value.element.num_attrs, attr = node->value.element.attrs;
        ...
        if (attr->value) width += strlen(attr->value) + 3;
        ...
        if (mxml_write_name(attr->name, p, putc_cb) < 0) return (-1);

        if (attr->value) {
          if ((*putc_cb)('=', p) < 0) return (-1);
          if ((*putc_cb)('\"', p) < 0) return (-1);
          if (mxml_write_string(attr->value, p, putc_cb) < 0) return (-1);
          if ((*putc_cb)('\"', p) < 0) return (-1);
        }
    ...
    case MXML_INTEGER:
      ...
      if (mxml_write_string(s, p, putc_cb) < 0) return (-1);

      col += strlen(s);
      break;

    case MXML_OPAQUE:
      if (mxml_write_string(node->value.opaque, p, putc_cb) < 0) return (-1);
    ...
    case MXML_CUSTOM:
      if (global->custom_save_cb) {
        ...
        if ((data = (*global->custom_save_cb)(node)) == NULL) return (-1);
