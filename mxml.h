...
typedef int (*mxml_sax_cb_t)(mxml_node_t *, mxml_sax_event_t, void *);
...
extern void mxmlAdd(mxml_node_t *parent, int where, mxml_node_t *child,
                    mxml_node_t *node);
...
extern mxml_node_t *mxmlIndexFind(mxml_index_t *ind, const char *element,
                                  const char *value);
...
extern int mxmlSaveString(mxml_node_t *node, char *buffer, int bufsize,
                          mxml_save_cb_t cb);
...
extern int mxmlSetTextf(mxml_node_t *node, int whitespace, const char *format,
                        ...)
#ifdef __GNUC__
    __attribute__((__format__(__printf__, 3, 4)))
#endif /* __GNUC__ */
