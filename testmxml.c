         ...
         char *argv[]) /* I - Command-line args */
{
  ...
  if (node->type != MXML_INTEGER) {
    fprintf(
        stderr, "ERROR: First child has type %s (%d), expected MXML_INTEGER!\n",
        node->type < MXML_ELEMENT || node->type > MXML_TEXT ? "UNKNOWN"
                                                            : types[node->type],
        node->type);
    mxmlDelete(tree);
    return (1);
  ...
  if (node->type != MXML_OPAQUE) {
    fprintf(
        stderr, "ERROR: Second child has type %s (%d), expected MXML_OPAQUE!\n",
        node->type < MXML_ELEMENT || node->type > MXML_TEXT ? "UNKNOWN"
                                                            : types[node->type],
        node->type);
    mxmlDelete(tree);
  ...
  if (!node->value.opaque || strcmp(node->value.opaque, "opaque")) {
    fprintf(stderr,
            "ERROR: Second child value is \"%s\", expected \"opaque\"!\n",
  ...
  if (node->type != MXML_REAL) {
    fprintf(
        stderr, "ERROR: Third child has type %s (%d), expected MXML_REAL!\n",
        node->type < MXML_ELEMENT || node->type > MXML_TEXT ? "UNKNOWN"
                                                            : types[node->type],
        node->type);
    mxmlDelete(tree);
  ...
  if (node->type != MXML_TEXT) {
    fprintf(
        stderr, "ERROR: Fourth child has type %s (%d), expected MXML_TEXT!\n",
        node->type < MXML_ELEMENT || node->type > MXML_TEXT ? "UNKNOWN"
                                                            : types[node->type],
        node->type);
    mxmlDelete(tree);
  ...
  if (!node->value.text.whitespace || !node->value.text.string ||
      strcmp(node->value.text.string, "text")) {
    fprintf(stderr,
  ...
  if (ind->num_nodes != 10) {
    fprintf(stderr,
            "ERROR: Index of all nodes contains %d "
            "nodes; expected 10!\n",
            ind->num_nodes);
    mxmlIndexDelete(ind);
    mxmlDelete(tree);
  ...
  if (ind->num_nodes != 3) {
    fprintf(stderr,
            "ERROR: Index of type attributes contains %d "
            "nodes; expected 3!\n",
            ind->num_nodes);
    mxmlIndexDelete(ind);
    mxmlDelete(tree);
  ...
  if (ind->num_nodes != 3) {
    fprintf(stderr,
            "ERROR: Index of elements and attributes contains %d "
            "nodes; expected 3!\n",
            ind->num_nodes);
    mxmlIndexDelete(ind);
    mxmlDelete(tree);
  ...
  if (argv[1][0] != '<') {
    ...
    if ((fd = open(buffer, O_WRONLY | O_CREAT | O_TRUNC | O_BINARY, 0666)) <
        0) {
      perror(buffer);
      mxmlDelete(tree);
      return (1);
  ...
  if (!strcmp(argv[1], "test.xml")) {
    ...
    if (event_counts[MXML_SAX_ELEMENT_CLOSE] != 20) {
      fprintf(stderr,
              "MXML_SAX_ELEMENT_CLOSE seen %d times, expected 20 times!\n",
    ...
    if (event_counts[MXML_SAX_ELEMENT_OPEN] != 20) {
      fprintf(stderr,
              "MXML_SAX_ELEMENT_OPEN seen %d times, expected 20 times!\n",
                  ...
                  int where)         /* I - Open or close tag? */
{
  ...
  if (!strcmp(name, "html") || !strcmp(name, "head") || !strcmp(name, "body") ||
      !strcmp(name, "pre") || !strcmp(name, "p") || !strcmp(name, "h1") ||
      !strcmp(name, "h2") || !strcmp(name, "h3") || !strcmp(name, "h4") ||
    ...
    if (where == MXML_WS_BEFORE_OPEN || where == MXML_WS_AFTER_CLOSE)
      return ("\n");
  } else if (!strcmp(name, "dl") || !strcmp(name, "ol") ||
             !strcmp(name, "ul")) {
    /*
    ...
    return ("\n");
  } else if (!strcmp(name, "dd") || !strcmp(name, "dt") ||
             !strcmp(name, "li")) {
    /*
    ...
    for (level = -1, parent = node->parent; parent;
