...
static void write_element(FILE *out, mxml_node_t *doc, mxml_node_t *element,
                          int mode);
static void write_file(FILE *out, const char *file);
static void write_function(FILE *out, mxml_node_t *doc, mxml_node_t *function,
                           int level);
static void write_html(const char *section, const char *title,
                       const char *footerfile, const char *headerfile,
                       const char *introfile, const char *cssfile,
                       const char *framefile, const char *docset,
                       const char *docversion, const char *feedname,
                       const char *feedurl, mxml_node_t *doc);
static void write_html_head(FILE *out, const char *section, const char *title,
                            const char *cssfile);
static void write_man(const char *man_name, const char *section,
                      ...
                      mxml_node_t *doc);
static void write_scu(FILE *out, mxml_node_t *doc, mxml_node_t *scut);
static void write_string(FILE *out, const char *s, int mode);
static void write_toc(FILE *out, mxml_node_t *doc, const char *introfile,
                      const char *target, int xml);
    ...
    if (!strcmp(argv[i], "--help")) {
      ...
      if (len > 4 && !strcmp(argv[i] + len - 4, ".xml")) {
        ...
        if (!doc) {
          ...
          if (!doc) doc = new_documentation(&mxmldoc);
        }
      } else {
        /*
         * Load source file...
  ...
  switch (mode) {
    case OUTPUT_HTML:
      ...
      write_html(section, title ? title : "Documentation", footerfile,
                 headerfile, introfile, cssfile, framefile, docset, docversion,
                 feedname, feedurl, mxmldoc);
      break;
                 ...
                 mxml_node_t *type)   /* I - Type nodes */
{
  ...
  if (!type || !type->child) return (NULL);
  ...
  for (node = type->child; node; node = node->next)
    if (!strcmp(node->value.text.string, "=")) break;
  ...
  if (node) {
    ...
    for (bufptr = buffer; node; bufptr += strlen(bufptr)) {
      if (node->value.text.whitespace && bufptr > buffer) *bufptr++ = ' ';
  ...
  if (type->last_child->value.text.string[0] == ')') {
    ...
    for (node = type->child; node; node = node->next)
      if (node->value.text.string[0] == '(') break;
                ...
                const char *name)  /* I - Name of element */
{
  ...
  for (node =
           mxmlFindElement(node, top, name, NULL, NULL,
                           node == top ? MXML_DESCEND_FIRST : MXML_NO_DESCEND);
       node;
...
static char *                                  /* O - Info from comment */
    get_comment_info(mxml_node_t *description) /* I - Description node */
{
  ...
  for (ptr = strchr(text, '@'); ptr; ptr = strchr(ptr + 1, '@')) {
    if (!strncmp(ptr, "@deprecated@", 12))
      ...
      if ((ptr = strchr(since, '@')) != NULL) *ptr = '\0';
              ...
              mxml_node_t *tree)    /* I - Function tree */
{
  ...
  static const char *states[] = /* State strings */
      {"STATE_NONE",        "STATE_PREPROCESSOR", "STATE_C_COMMENT",
       "STATE_CXX_COMMENT", "STATE_STRING",       "STATE_CHARACTER",
       "STATE_IDENTIFIER"};
#endif /* DEBUG > 1 */
    ...
    switch (state) {
        ...
        switch (ch) {
          ...
          case '{':
            ...
            if (function) {
              ...
              else if (structclass && type->child && type->child->next &&
                       type->child->next->next) {
                for (bufptr = buffer, node = type->child->next->next; node;
              ...
              if (typedefnode && comment->last_child) {
                ...
                mxmlNewText(comment, 0, comment->last_child->value.text.string);
          ...
          case '}':
            ...
            if (structclass) scope = NULL;

            if (!typedefnode) enumeration = NULL;
          ...
          case ';':
            ...
            fputs("Identifier: <<<< ; >>>\n", stderr);
            fprintf(stderr,
                    "    enumeration=%p, function=%p, type=%p, type->child=%p, "
                    "typedefnode=%p\n",
            ...
            if (type) {
              ...
              if (type->child &&
                ...
                if (type->child != node) mxmlDelete(type->child);

                mxmlDelete(node);

                if (type->child) type->child->value.text.whitespace = 0;
      ...
      case STATE_C_COMMENT: /* Inside a C comment */
        switch (ch) {
                ...
                if (ch == '/') {
                  *bufptr = '\0';
                  ...
                  if (comment->child != comment->last_child) {
                    ...
                    fprintf(
                        stderr,
                        "    removing comment %p(%20.20s), last comment "
                        "%p(%20.20s)...\n",
                        comment->child,
                  ...
                  if (variable) {
                    ...
                    if (strstr(buffer, "@private@")) {
                      ...
                      if (structclass) {
                        description =
                            mxmlNewElement(structclass, "description");
                        update_comment(structclass,
                                       mxmlNewText(description, 0, buffer));
                      } else if (enumeration) {
                        description =
                            mxmlNewElement(enumeration, "description");
                    ...
                    typedefnode = NULL;
                             ...
                             !mxmlFindElement(tree, tree, "description", NULL,
...
#endif /* DEBUG */
                    mxmlNewText(comment, 0, buffer);
                    update_comment(tree, mxmlNewText(description, 0, buffer));
                  } else {
#ifdef DEBUG
                    fprintf(
                        stderr,
                        "    before adding comment, child=%p, last_child=%p\n",
                        comment->child, comment->last_child);
#endif /* DEBUG */
                    mxmlNewText(comment, 0, buffer);
#ifdef DEBUG
                    fprintf(
                        stderr,
                        "    after adding comment, child=%p, last_child=%p\n",
                        comment->child, comment->last_child);
#endif /* DEBUG */
                  }
#ifdef DEBUG
              ...
              *bufptr = '\0';
              ...
              if (variable) {
...
#endif /* DEBUG */
                  mxmlNewText(comment, 0, buffer);
                  update_comment(variable, mxmlNewText(description, 0, buffer));
                }
...
#endif /* DEBUG */
                  mxmlNewText(comment, 0, buffer);
                  update_comment(constant, mxmlNewText(description, 0, buffer));
                }
                ...
                typedefnode = NULL;
                         ...
                         !mxmlFindElement(tree, tree, "description", NULL, NULL,
                                          MXML_DESCEND_FIRST)) {
                description = mxmlNewElement(tree, "description");
#ifdef DEBUG
                fprintf(stderr, "    adding comment %p/%p to parent...\n",
          ...
          default:
            if (ch == ' ' && bufptr == buffer) break;

            if (bufptr < (buffer + sizeof(buffer) - 1)) *bufptr++ = ch;
            break;
        }
        break;
...
#endif /* DEBUG */
              mxmlNewText(comment, 0, buffer);
              update_comment(typedefnode, mxmlNewText(description, 0, buffer));
        ...
        *bufptr++ = ch;
        ...
        if (ch == '\\')
          ...
          *bufptr = '\0';
          ...
          if (type) mxmlNewText(type, type->child != NULL, buffer);
      ...
      case STATE_IDENTIFIER: /* Inside a keyword or identifier */
        if (isalnum(ch) || ch == '_' || ch == '[' || ch == ']' ||
            (ch == ',' &&
             (parens > 1 || (type && !enumeration && !function))) ||
            ch == ':' || ch == '.' || ch == '~') {
          if (bufptr < (buffer + sizeof(buffer) - 1)) *bufptr++ = ch;
        } else {
          ungetc(ch, fp);
          *bufptr = '\0';
...
#ifdef DEBUG
          fprintf(stderr,
                  "    braces=%d, type=%p, type->child=%p, buffer=\"%s\"\n",
          ...
          if (!braces) {
            if (!type || !type->child) {
              if (!strcmp(tree->value.element.name, "class")) {
                if (!strcmp(buffer, "public") || !strcmp(buffer, "public:")) {
                  scope = "public";
#ifdef DEBUG
                  fputs("    scope = public\n", stderr);
#endif /* DEBUG */
            ...
            if (!type) type = mxmlNewElement(MXML_NO_PARENT, "type");

#ifdef DEBUG
            fprintf(
                stderr,
                "    function=%p (%s), type->child=%p, ch='%c', parens=%d\n",
                function,
              ...
              if ((bufptr = strchr(buffer, ':')) != NULL && bufptr[1] == ':') {
                *bufptr = '\0';
                ...
                if ((fstructclass =
                  ...
                  fstructclass = mxmlFindElement(tree, tree, "struct", "name",
                                                 buffer, MXML_DESCEND_FIRST);
              } else
                bufptr = buffer;

              mxmlElementSetAttr(function, "name", bufptr);

              if (scope) mxmlElementSetAttr(function, "scope", scope);
...
#ifdef DEBUG
              fprintf(stderr,
                      "    got semicolon, typedefnode=%p, structclass=%p\n",
 ...
 */
                ...
                if (scope) mxmlElementSetAttr(variable, "scope", scope);
              }
            } else {
#ifdef DEBUG
                      ...
                      mxml_node_t *node) /* I - Node to add */
{
  ...
  if (!tree || !node || node->parent == tree) return;

  /*
   * Get the node name...
   */

  if ((nodename = mxmlElementGetAttr(node, "name")) == NULL) return;

  if (nodename[0] == '_') return; /* Hide private names */
  ...
  if ((temp = mxmlFindElement(tree, tree, node->value.element.name, "name",
                              nodename, MXML_DESCEND_FIRST)) != NULL) {
    /*
  ...
  for (temp = tree->child; temp; temp = temp->next) {
    ...
    if ((tempname = mxmlElementGetAttr(temp, "name")) == NULL) continue;

#if DEBUG > 1
    fprintf(stderr, "        tempname=%p (\"%s\")\n", tempname, tempname);
#endif /* DEBUG > 1 */

    if (strcmp(nodename, tempname) < 0) break;
  }
                           ...
                           mxml_node_t *comment) /* I - Comment node */
{
  ...
  for (ptr = strstr(comment->value.text.string, "\\/"); ptr;
       ptr = strstr(ptr, "\\/"))
  ...
  for (ptr =
           comment->value.text.string + strlen(comment->value.text.string) - 1;
       ptr > comment->value.text.string && *ptr == '*'; ptr--)
    *ptr = '\0';
...
{
  if (option) printf("mxmldoc: Bad option \"%s\"!\n\n", option);
                          ...
                          int mode)             /* I - Output mode */
{
  ...
  for (node = element->child; node;
    ...
    if (node->type == MXML_TEXT) {
      if (node->value.text.whitespace) putc(' ', out);
                           ...
                           int level)             /* I - Base heading level */
{
  ...
  description = mxmlFindElement(function, function, "description", NULL, NULL,
                                MXML_DESCEND_FIRST);

  fprintf(out, "<h%d class=\"%s\">%s<a name=\"%s\">%s</a></h%d>\n", level,
          level == 3 ? "function" : "method", get_comment_info(description),
          name, name, level);

  if (description) write_description(out, description, "p", 1);

  fputs("<p class=\"code\">\n", out);

  arg = mxmlFindElement(function, function, "returnvalue", NULL, NULL,
                        MXML_DESCEND_FIRST);

  if (arg)
    write_element(out, doc, mxmlFindElement(arg, arg, "type", NULL, NULL,
                                            MXML_DESCEND_FIRST),
                  OUTPUT_HTML);
  else
    fputs("void ", out);
  ...
  for (arg = mxmlFindElement(function, function, "argument", NULL, NULL,
                             MXML_DESCEND_FIRST),
      prefix = '(';
       arg; arg = mxmlFindElement(arg, function, "argument", NULL, NULL,
                                  MXML_NO_DESCEND),
      prefix = ',') {
    type = mxmlFindElement(arg, arg, "type", NULL, NULL, MXML_DESCEND_FIRST);

    fprintf(out, "%c<br>\n&nbsp;&nbsp;&nbsp;&nbsp;", prefix);
    if (type->child) write_element(out, doc, type, OUTPUT_HTML);
 ...
 */
    ...
    if (!access(docset, 0) && !remove_directory(docset)) return;
    ...
    snprintf(filename, sizeof(filename),
             "%s/Contents/Resources/Documentation/index.html", docset);
    if ((out = fopen(filename, "w")) == NULL) {
      fprintf(stderr, "mxmldoc: Unable to create \"%s\": %s\n", filename,
              strerror(errno));
      return;
  ...
  if (!framefile) write_toc(out, doc, introfile, NULL, 0);

  /*
   * Intro...
   */

  if (introfile) write_file(out, introfile);
  ...
  if ((scut = find_public(doc, doc, "typedef")) != NULL) {
    ...
    while (scut) {
      ...
      description = mxmlFindElement(scut, scut, "description", NULL, NULL,
                                    MXML_DESCEND_FIRST);
      ...
      type =
          mxmlFindElement(scut, scut, "type", NULL, NULL, MXML_DESCEND_FIRST);

      for (type = type->child; type; type = type->next)
        if (!strcmp(type->value.text.string, "("))
          break;
        else {
          if (type->value.text.whitespace) putc(' ', out);
      ...
      if (type) {
        ...
        type =
            mxmlFindElement(scut, scut, "type", NULL, NULL, MXML_DESCEND_FIRST);
        if (type->last_child->value.text.string[0] != '*') putc(' ', out);
  ...
  if ((arg = find_public(doc, doc, "variable")) != NULL) {
    ...
    while (arg) {
      ...
      description = mxmlFindElement(arg, arg, "description", NULL, NULL,
                                    MXML_DESCEND_FIRST);
  ...
  if ((scut = find_public(doc, doc, "enumeration")) != NULL) {
    ...
    while (scut) {
      ...
      for (arg = mxmlFindElement(scut, scut, "constant", NULL, NULL,
                                 MXML_DESCEND_FIRST);
           arg; arg = mxmlFindElement(arg, scut, "constant", NULL, NULL,
                                      MXML_NO_DESCEND)) {
        description = mxmlFindElement(arg, arg, "description", NULL, NULL,
                                      MXML_DESCEND_FIRST);
                            ...
                            const char *cssfile) /* I - Stylesheet */
{
  ...
  if (section) fprintf(out, "<!-- SECTION: %s -->\n", section);
                      ...
                      mxml_node_t *doc)       /* I - XML documentation */
{
  ...
  if (introfile) write_file(stdout, introfile);
  ...
  if (find_public(doc, doc, "class")) {
    ...
    for (scut = find_public(doc, doc, "class"); scut;
         scut = find_public(scut, doc, "class")) {
      ...
      for (i = 0; i < 3; i++) {
        ...
        for (arg = mxmlFindElement(scut, scut, "variable", "scope", scopes[i],
                                   MXML_DESCEND_FIRST);
             arg; arg = mxmlFindElement(arg, scut, "variable", "scope",
        ...
        for (function = mxmlFindElement(scut, scut, "function", "scope",
          ...
          if (arg)
            write_element(stdout, doc,
                          mxmlFindElement(arg, arg, "type", NULL, NULL,
                                          MXML_DESCEND_FIRST),
                          OUTPUT_MAN);
          ...
          for (arg = mxmlFindElement(function, function, "argument", NULL, NULL,
            ...
            if (type->child) write_element(stdout, doc, type, OUTPUT_MAN);
  ...
  if (find_public(doc, doc, "enumeration")) {
    ...
    for (scut = find_public(doc, doc, "enumeration"); scut;
         scut = find_public(scut, doc, "enumeration")) {
      name = mxmlElementGetAttr(scut, "name");
      description = mxmlFindElement(scut, scut, "description", NULL, NULL,
                                    MXML_DESCEND_FIRST);
  ...
  if (find_public(doc, doc, "function")) {
    ...
    for (function = find_public(doc, doc, "function"); function;
         function = find_public(function, doc, "function")) {
      name = mxmlElementGetAttr(function, "name");
      ...
      if (arg)
        write_element(stdout, doc, mxmlFindElement(arg, arg, "type", NULL, NULL,
                                                   MXML_DESCEND_FIRST),
                      OUTPUT_MAN);
      else
        fputs("void", stdout);
      ...
      for (arg = mxmlFindElement(function, function, "argument", NULL, NULL,
          ...
          prefix = ',') {
        type =
            mxmlFindElement(arg, arg, "type", NULL, NULL, MXML_DESCEND_FIRST);
  ...
  if (find_public(doc, doc, "struct")) {
    ...
    for (scut = find_public(doc, doc, "struct"); scut;
         scut = find_public(scut, doc, "struct")) {
      cname = mxmlElementGetAttr(scut, "name");
      description = mxmlFindElement(scut, scut, "description", NULL, NULL,
                                    MXML_DESCEND_FIRST);
      ...
      for (arg = mxmlFindElement(scut, scut, "variable", NULL, NULL,
                                 MXML_DESCEND_FIRST);
           arg; arg = mxmlFindElement(arg, scut, "variable", NULL, NULL,
                                      MXML_NO_DESCEND)) {
        printf("  ");
        write_element(stdout, doc, mxmlFindElement(arg, arg, "type", NULL, NULL,
                                                   MXML_DESCEND_FIRST),
      ...
      for (function = mxmlFindElement(scut, scut, "function", NULL, NULL,
                                      MXML_DESCEND_FIRST);
           function; function = mxmlFindElement(function, scut, "function",
  ...
  if (find_public(doc, doc, "typedef")) {
    ...
    for (scut = find_public(doc, doc, "typedef"); scut;
         scut = find_public(scut, doc, "typedef")) {
      name = mxmlElementGetAttr(scut, "name");
      description = mxmlFindElement(scut, scut, "description", NULL, NULL,
                                    MXML_DESCEND_FIRST);
      ...
      for (type = type->child; type; type = type->next)
        if (!strcmp(type->value.text.string, "("))
          ...
          if (type->value.text.whitespace) putchar(' ');
  ...
  if (find_public(doc, doc, "union")) {
    ...
    for (scut = find_public(doc, doc, "union"); scut;
         scut = find_public(scut, doc, "union")) {
      name = mxmlElementGetAttr(scut, "name");
      description = mxmlFindElement(scut, scut, "description", NULL, NULL,
                                    MXML_DESCEND_FIRST);
  ...
  if (find_public(doc, doc, "variable")) {
    ...
    for (arg = find_public(doc, doc, "variable"); arg;
         arg = find_public(arg, doc, "variable")) {
      name = mxmlElementGetAttr(arg, "name");
      description = mxmlFindElement(arg, arg, "description", NULL, NULL,
                                    MXML_DESCEND_FIRST);
                      ...
                      mxml_node_t *scut) /* I - Structure, class, or union */
{
  ...
  for (i = 0; i < maxscope; i++) {
    ...
    for (arg = mxmlFindElement(scut, scut, "variable", NULL, NULL,
         ...
         arg; arg = mxmlFindElement(arg, scut, "variable", NULL, NULL,
                                    MXML_NO_DESCEND)) {
      if (maxscope > 1 && ((scope = mxmlElementGetAttr(arg, "scope")) == NULL ||
    ...
    for (function = mxmlFindElement(scut, scut, "function", NULL, NULL,
                                    MXML_DESCEND_FIRST);
         function; function = mxmlFindElement(function, scut, "function", NULL,
  ...
  if (introfile && (fp = fopen(introfile, "r")) != NULL) {
    ...
    while (fgets(line, sizeof(line), fp)) {
      ...
      for (ptr = strchr(line, '<'); ptr; ptr = strchr(ptr + 1, '<'))
        if (!strncmp(ptr, "<A NAME=", 8) || !strncmp(ptr, "<a name=", 8)) break;
      ...
      if (*ptr == '\'' || *ptr == '\"') {
        ...
        *ptr++ = '\0';
        ...
        while (*ptr && *ptr != '>' && !isspace(*ptr & 255)) ptr++;
      ...
      if (xml) {
        ...
        while (*ptr) {
          if (inelement) {
            ...
            if (!strncmp(ptr, "</A>", 4) || !strncmp(ptr, "</a>", 4)) break;
    ...
    if (level > '1') {
      if (xml) {
        ...
        if (level == '3') fputs("</Subnodes></Node>\n", out);
      } else {
        fputs("</li>\n", out);

        if (level == '3') fputs("</ul></li>\n", out);
      }
    }
  ...
  if ((scut = find_public(doc, doc, "class")) != NULL) {
    ...
    while (scut) {
      ...
      if (xml) {
        ...
        fprintf(out, "\t<li><a href=\"%s#%s\"%s title=\"", target ? target : "",
                name, targetattr);
      ...
      if ((function = find_public(scut, scut, "function")) != NULL) {
        while (function) {
          ...
          fprintf(out,
                  ...
                  "<Path>Documentation/%s</Path>\n"
                  "<Anchor>%s.%s</Anchor>\n"
                  "<TokenIdentifier>//apple_ref/cpp/clm/%s/%s",
                  path, cename, name, cename, name);

          arg = mxmlFindElement(function, function, "returnvalue", NULL, NULL,
                                MXML_DESCEND_FIRST);

          if (arg && (type = mxmlFindElement(arg, arg, "type", NULL, NULL,
                                             MXML_DESCEND_FIRST)) != NULL) {
            for (node = type->child; node; node = node->next)
          ...
          for (arg = mxmlFindElement(function, function, "argument", NULL, NULL,
            ...
            if (type->child) write_element(out, doc, type, OUTPUT_XML);
          ...
          int where)         /* I - Where value */
{
  ...
  switch (where) {
    case MXML_WS_BEFORE_CLOSE:
      if (strcmp(name, "argument") && strcmp(name, "class") &&
          strcmp(name, "constant") && strcmp(name, "enumeration") &&
          strcmp(name, "function") && strcmp(name, "mxmldoc") &&
          strcmp(name, "namespace") && strcmp(name, "returnvalue") &&
          strcmp(name, "struct") && strcmp(name, "typedef") &&
          strcmp(name, "union") && strcmp(name, "variable"))
        return (NULL);
